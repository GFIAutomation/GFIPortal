    ...
    public class HomeController : Controller
    {
        ...
        public ActionResult Project(string language)
        {
            ...
            try
            {
                ...
                var ProjectList = (from p in db.Project
                                    ...
                                    select new ProjectViewModel { Id = p.id, ProjectName = p.name, ProjectDescription = p.description, UrlImage= p.logo_url }).ToList();
        ...
        public ActionResult Dashboard(string language)
        {
            ...
            if (project_id > 0)
            {
                ...
                var dataLastFiveBuild = (from p in db.Project
                                         join r in db.Report on p.id equals project_id
                                         orderby r.date_end descending
                ...
                obj.DataLastFiveBuild = dataLastFiveBuild;
        ...
        [Authorize]
        public ActionResult DashboardReport(string language)
        {
            changeLanguage(language);
            ...
            if (project_id > 0)
            {
                ...
                List<String> dateLista = new List<String>();
                int j = 0;
                foreach (var date in dataLastFiveBuild)
                {
                    var p = date.ToString().Remove(6);
                    j++;
                    dateLista.Add(p);
